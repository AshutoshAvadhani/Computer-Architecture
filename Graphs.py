...
for x in range(len(assoc)):
    for y in range(len(L1_size)):
        if assoc[x] == "full":
            command = "python MainFunction.py 32 {} {} 0 0 0 0 Traces/gcc_trace.txt".format(L1_size[y], int(L1_size[y] / 32))
        else:
            command = "python MainFunction.py 32 {} {} 0 0 0 0 Traces/gcc_trace.txt".format(L1_size[y], assoc[x])
        print(command)
...
plt.legend()
plt.title("Size and Assoc study") 
plt.savefig(r'D:\UCF\ACA\Assignment\1\Programming Assignment 1\Graphs\Graph1.png')
plt.close()
...
plt.legend()
plt.title("AAT Study")
plt.savefig(r'D:\UCF\ACA\Assignment\1\Programming Assignment 1\Graphs\Graph2.png')
plt.close()
...
for x in range(len(rep_pol)):
    for y in range(len(L1_size)):
        command = "python MainFunction.py 32 {} 4 0 0 {} 0 Traces/gcc_trace.txt".format(L1_size[y], rep_pol[x])
        output = subprocess.check_output(command, shell=True)
        output = output.decode("utf-8")
        val = re.findall("L1 miss rate:\\s+(\\d.\\d+)+", output)[0]
        miss_rates[x][y] = val
...
plt.legend()
plt.title("Replacement Policy Study")
plt.savefig(r'D:\UCF\ACA\Assignment\1\Programming Assignment 1\Graphs\Graph3.png')
plt.close()
...
for x in range(len(inc_pol)):
    for y in range(len(L2_size)):
        command = "python MainFunction.py 32 1024 4 {} 8 0 {} Traces/gcc_trace.txt".format(L2_size[y], inc_pol[x])
        output = subprocess.check_output(command, shell=True)
        output = output.decode("utf-8")
...
plt.legend()
plt.title("Inclusion Property Study")
plt.savefig(r'D:\UCF\ACA\Assignment\1\Programming Assignment 1\Graphs\Graph4.png')
plt.close()